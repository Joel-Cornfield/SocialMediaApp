...
exports.getFollowing = [
    ...
    asyncHandler(async (req, res, next) => {
        ...
        const exist = await prisma.user.findUnique({ where: { id } });
        if (!exist) return res.status(404).json({ error: `UserID ${id} does not exist.` });
        const following = await get_following(id);
...
exports.patchProfile = [
    ...
    param("userId", "invalid user id").trim(),
    body("username")
        .trim()
        .isLength({ min: 2, max: 25 })
        .withMessage("Username between 2-25 characters")
        .matches(/^[a-zA-Z0-9_.]*$/)
    ...
    body("bio").trim().isLength({ max: 350 }).withMessage("Max bio length is 350"),
    body("website").optional({ nullable: true, checkFalsy: true }).trim().isURL().withMessage("Website not a valid URL"),
    body("github").optional({ nullable: true, checkFalsy: true }).trim().isURL().withMessage("GitHub not a valid URL"),
    validationHandle,
    asyncHandler(async (req, res, next) => {
        ...
        if (req.file) {
            ...
            if (mimetype !== "image") throw new myError("Only image uploads allowed for profilePicture", 400);
            if (req.file.size > 1024 * 1024 * 8) throw new myError("File size exceeds 8MB", 400);

            const result = await uploadStream(req.file.buffer, "bingus_pfp");
            if (user_profile.pfp_public_id) {
                await deleteFile(user_profile.pfp_public_id);
            }
...
exports.patchSettings = [
    body("theme").trim().isIn(["dark", "light"]),
    validationHandle,
    asyncHandler(async (req, res, next) => {
        ...
        const settings = await prisma.userSetting.upsert({
            where: { userId: req.user.id },
            update: { isDarkMode },
            create: { isDarkMode, userId: req.user.id }
        });
